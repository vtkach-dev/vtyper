...
const closeCrossEnd = document.getElementById('close-cross-end');
const closeCrossTab = document.getElementById('close-cross-tab');
...
const e2focus = document.getElementById('e2focus')
...
langButton.element.onclick = function() {
    // let newLang = (currLang == 'ru' ? 'en': 'ru');
    // currLang = newLang;
    // init();
    
    // this.children[0].innerText = newLang;
...
function setText(text, numOfWords) {
    ...
    startOverlayText.innerText = langs[currLang]['start-overlay-keyboard']
    ...
    if (!(currLang in cachedLayouts)) {
        fetch(`${window.location.href}api/layouts?lang=${currLang}`)
            ...
            .then(data => {
                ...
                renderLayout(tabContent, headerNavList[0].id, currLang)
            })
    } else {
        renderLayout(tabContent, headerNavList[0].id, currLang)
    }
...
headerNav.addEventListener('click', function (e) {
    ...
    if (target.className == 'nav-button') {       
        ...
        tabOverlay.setStyle(1)

        renderLayout(tabContent, target.id, currLang)
...
function closeTabOverlay() {
    tabOverlay.setStyle(0)
    e2focus.focus()
}

tabOverlay.element.addEventListener('click', function (event) {
    if (event.target == tabOverlay.element || event.target == closeCrossTab) {
        tabOverlay.setStyle(0)
    }
})
...
document.addEventListener('click', function(e) {
    if ( (
        e.target.id == 'start-overlay' ||
        e.target.id == 'text' ||
        e.target.id == 'start-overlay-text'
    ) && typer.kb_state == 1) {
        e2focus.focus()
    }
})

e2focus.addEventListener('input', function(e) {
    e2focus.value = ' '

    generalKeyHandler(e.data)
    // console.log(e.data);
})

endOverlay.element.addEventListener('click', function (event) {
    if (event.target == endOverlay.element || event.target == closeCrossEnd) {
        endStats.setStyle(2)
